...
import { getNewTableNzgos } from 'apiSdk/new-table-nzgos';
import { NewTableNzgoInterface } from 'interfaces/new-table-nzgo';

import { NewTableNzgoListPage as ShowCaseList } from 'pages/new-table-nzgos';
...
function DashboardPage() {
  ...
  const newTableNzgoStat = useSWR<PaginatedInterface<NewTableNzgoInterface>>(
    () => `/new-table-nzgos`,
    () => getNewTableNzgos({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'New Table Nzgos',
      result: newTableNzgoStat?.data?.totalCount,
      isLoading: newTableNzgoStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest New Table Nzgos
                </Text>
                <NextLink href={`new-table-nzgos`} passHref legacyBehavior>
                  <Button
